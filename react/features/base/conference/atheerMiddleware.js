...
MiddlewareRegistry.register(store => next => action => {
    ...
    case SET_DISPLAY_NAME:
        return _updateParticipantDisplayName(store, next, action);
    }
...
function _updateParticipantDisplayName({ getState }, next, action) {
    const { conference } = getState()['features/base/conference'];
    const result = next(action);

    if (conference) {
        conference.setDisplayName(action.participant.displayName);
    }
